...
int FallDet::init(const std::string& modelPath) {
   std::cout << "Loading FallDet model " << modelPath << " ..." << std::endl;
...
rknn_context* FallDet::get_rknn_context() {
    // 返回 RKNN context
    return nullptr;
}
...
FallDetResult FallDet::getResult() const {
    return result_;
}
